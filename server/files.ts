...
const UPLOAD_ROOT = path.resolve(process.env.FILE_STORAGE_DIR || "uploads");
...
function assertValidFile(mimeType: string, buffer: Buffer): void {
  ...
  const signatures: Record<string, (value: Buffer) => boolean> = {
    "application/pdf": (v) => v.subarray(0, 5).toString("ascii") === "%PDF-",
    "image/jpeg": (v) => v.length >= 3 && v[0] === 0xff && v[1] === 0xd8 && v[2] === 0xff,
    "image/png": (v) => v.subarray(0, 8).equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])),
    "image/gif": (v) => ["GIF87a", "GIF89a"].includes(v.subarray(0, 6).toString("ascii")),
    "image/webp": (v) => v.subarray(0, 4).toString("ascii") === "RIFF" && v.subarray(8, 12).toString("ascii") === "WEBP",
  };
  if (!signatures[mimeType]?.(buffer)) throw new Error("Filens indhold matcher ikke den angivne filtype.");
  if (buffer.length > MAX_FILE_SIZE) {
    throw new Error("Filen er for stor. Maksimal filstørrelse er 10 MB.");
  }
}

function extensionFromName(_fileName: string, mimeType: string): string {
...
function diskPath(storageKey: string): string {
  ...
  if (!filePath.startsWith(`${path.resolve(UPLOAD_ROOT)}${path.sep}`)) throw new Error("Ugyldig filsti.");
  return filePath;
}
