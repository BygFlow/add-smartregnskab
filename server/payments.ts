...
export async function chargeInvoice(
  ...
  if (!provider.configured && process.env.NODE_ENV === "production") {
    return {
      ok: false,
      pending: false,
      simulated: false,
      payment: null,
      message: `Betalingsudbyderen "${provider.label}" er ikke konfigureret. Ingen opkrævning er foretaget.`,
    };
  }
  ...
  if (!result.simulated) await settleInvoice(invoice, savedPayment, false);
  return {
    ok: true,
    pending: false,
    simulated: result.simulated,
