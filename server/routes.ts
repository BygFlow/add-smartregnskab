...
import { allocateDocumentNumber, createEncryptedBackup, createInvoiceWithItems, createJournalWithLines, createTenantBackup } from "./storage";
...
function validate<T>(schema: SafeParse<T>, data: unknown): T {
  ...
  if (data && typeof data === "object" && result.data && typeof result.data === "object") {
    const source = data as Record<string, unknown>;
    const managed: Record<string, unknown> = {};
    for (const key of ["companyId", "createdAt", "updatedAt", "uploadedBy"]) {
      if (Object.prototype.hasOwnProperty.call(source, key)) managed[key] = source[key];
    }
    return { ...(result.data as object), ...managed } as T;
  }
  return result.data;
}
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post("/api/signup", h(async (req, res) => {
    ...
    res.status(201).json({
      ...
      demoToken: emailConfigured() ? undefined : token,
    });
  }));
  ...
  app.post("/api/auth/verify/resend", h(async (req, res) => {
  ...
  app.post("/api/auth/logout", h(async (req, res) => {
    if (req.auth?.token) await storage.deleteSession(req.auth.token);
    res.status(204).send();
  }));
  ...
  app.get("/api/employees", h(async (req, res) => {
    res.json(await storage.getEmployees(tenantId(req)));
  }));
  ...
  app.get("/api/customers", h(async (req, res) => {
    res.json(await storage.getCustomers(tenantId(req)));
  }));
  ...
  app.get("/api/tasks", h(async (req, res) => {
    ...
    if (req.auth?.role === "kunde") {
      res.json(allTasks.filter(t => t.customerId === req.auth!.user.customerId));
      return;
    }
    res.json(allTasks);
  }));
  app.get("/api/tasks/customer/:customerId", h(async (req, res) => {
    if (req.auth?.role === "kunde" && Number(req.params.customerId) !== req.auth.user.customerId) {
      return res.status(404).json({ error: "Kunden blev ikke fundet." });
    }
  ...
  app.get("/api/tasks/:id", h(async (req, res) => {
    ...
    if (req.auth?.role === "assistent" && item.employeeId !== req.auth.employeeId) return res.status(404).json({ error: "Opgaven blev ikke fundet." });
    if (req.auth?.role === "kunde" && item.customerId !== req.auth.user.customerId) return res.status(404).json({ error: "Opgaven blev ikke fundet." });
    res.json(item);
  }));
  ...
  app.patch("/api/tasks/:id", h(async (req, res) => {
    ...
    if (req.auth?.role === "assistent" && existing.employeeId !== req.auth.employeeId) {
      return res.status(403).json({ error: "Du kan kun ændre egne tildelte opgaver." });
    }
    if (req.auth?.role === "kunde") return res.status(403).json({ error: "Kunder kan ikke ændre opgaver." });
  ...
  app.get("/api/tasks/:id/notes", h(async (req, res) => {
    ...
    if (req.auth?.role === "assistent" && task.employeeId !== req.auth.employeeId) return res.status(404).json({ error: "Opgaven blev ikke fundet." });
    if (req.auth?.role === "kunde" && task.customerId !== req.auth.user.customerId) return res.status(404).json({ error: "Opgaven blev ikke fundet." });
  ...
  app.post("/api/time-entries", requireRole("leder", "holdleder", "platform_admin"), h(async (req, res) => {
    const cid = tenantId(req);
    const geo = await evaluateGeofence(cid, req.body.taskId, req.body.checkInLat, req.body.checkInLng);
  ...
  app.get("/api/absences", requireFeature("fravaer"), h(async (req, res) => {
    const rows = await storage.getAbsences(tenantId(req));
    if (req.auth?.role === "assistent") return res.json(rows.filter((row) => row.employeeId === req.auth!.employeeId));
    res.json(rows);
  }));
  app.post("/api/absences", requireFeature("fravaer"), h(async (req, res) => {
    const cid = tenantId(req);
    const body = { ...req.body, companyId: cid };
    if (req.auth?.role === "assistent") body.employeeId = req.auth.employeeId;
  ...
  app.patch("/api/absences/:id", requireFeature("fravaer"), h(async (req, res) => {
    ...
    if (!isManager && existing.employeeId !== req.auth!.employeeId) {
      return res.status(403).json({ error: "Du kan kun ændre dit eget fravær." });
    }
    const patch: Record<string, unknown> = { ...req.body };
    delete patch.companyId;
    if (!isManager) {
  ...
  app.get("/api/shifts", requireFeature("vagtplan"), h(async (req, res) => {
    ...
    const rows = await storage.getShifts(tenantId(req), from, to);
    if (req.auth?.role === "assistent") return res.json(rows.filter((row) => row.employeeId === req.auth!.employeeId));
    if (req.auth?.role === "kunde") return res.json(rows.filter((row) => row.customerId === req.auth!.user.customerId));
    res.json(rows);
  }));
  ...
  app.get("/api/attachments", h(async (req, res) => {
    ...
    const cid = tenantId(req);
    const rows = await storage.getAttachments(cid, taskId ? Number(taskId) : undefined);
    if (req.auth?.role === "assistent" || req.auth?.role === "kunde") {
      const allowedTasks = (await storage.getTasks(cid)).filter((task) =>
        req.auth!.role === "assistent"
          ? task.employeeId === req.auth!.employeeId
          : task.customerId === req.auth!.user.customerId,
      );
      const ids = new Set(allowedTasks.map((task) => task.id));
      return res.json(rows.filter((row) => row.taskId != null && ids.has(row.taskId)));
    }
    res.json(rows);
  }));
  app.post("/api/attachments", requireFeature("fotodokumentation"), h(async (req, res) => {
    ...
    if (taskId !== null) {
      const task = await storage.getTask(taskId, cid);
      const allowed = task && (req.auth?.role === "assistent"
        ? task.employeeId === req.auth.employeeId
        : req.auth?.role === "kunde" ? task.customerId === req.auth.user.customerId : true);
      if (!allowed) return res.status(404).json({ error: "Opgaven blev ikke fundet." });
    } else if (req.auth?.role === "assistent" || req.auth?.role === "kunde") {
      return res.status(400).json({ error: "Bilaget skal knyttes til en opgave." });
    }
    const decoded = decodeDataUrl(dataUrl);
    const saved = await saveFile({
      companyId: cid,
      fileName: String(req.body.fileName ?? "bilag"),
      mimeType: decoded.mimeType,
      buffer: decoded.buffer,
    });
    const data = validate(insertAttachmentSchema, {
      ...req.body, companyId: cid,
      taskId, mimeType: decoded.mimeType, sizeBytes: saved.sizeBytes,
      dataUrl: null, storage: saved.storage, storageKey: saved.storageKey,
      uploadedBy: req.auth!.user.id, createdAt: nowIso(),
    });
    let item;
    try {
      item = await storage.createAttachment(data);
    } catch (error) {
  ...
  app.delete("/api/attachments/:id", h(async (req, res) => {
    ...
    if ((req.auth?.role === "assistent" || req.auth?.role === "kunde") && existing.taskId != null) {
      const task = await storage.getTask(existing.taskId, tenantId(req));
      const allowed = task && (req.auth.role === "assistent"
        ? task.employeeId === req.auth.employeeId
        : task.customerId === req.auth.user.customerId);
      if (!allowed) return res.status(404).json({ error: "Billedet blev ikke fundet." });
    }
    if (existing.storageKey) await deleteFile(existing.storage, existing.storageKey);
  ...
  app.get("/api/notifications", h(async (req, res) => {
    const rows = await storage.getNotifications(tenantId(req));
    if (req.auth?.role === "kunde" || req.auth?.role === "assistent") {
      return res.json(rows.filter((row) => row.userId === req.auth!.userId));
    }
    res.json(rows);
  }));
  ...
  app.get("/api/invoices", h(async (req, res) => {
    const rows = await storage.getInvoices(tenantId(req));
    if (req.auth?.role === "kunde") return res.json(rows.filter((row) => row.customerId === req.auth!.user.customerId));
    res.json(rows);
  }));
  app.get("/api/invoices/overdue", h(async (req, res) => {
    const list = await overdueInvoices(tenantId(req), today());
    res.json(list.map((i) => ({ ...i, daysOverdue: daysBetween(i.dueDate!, today()) })));
  }));
  ...
  app.get("/api/invoices/:id", h(async (req, res) => {
    ...
    if (req.auth?.role === "kunde" && inv.customerId !== req.auth.user.customerId) {
      return res.status(404).json({ error: "Fakturaen blev ikke fundet." });
    }
  ...
  app.post("/api/invoices", requireRole("leder", "platform_admin"), h(async (req, res) => {
    ...
    const sequence = allocateDocumentNumber(cid, "invoice");
    const invoiceData = validate(insertInvoiceSchema, {
      companyId: cid,
      customerId: cust.id,
      invoiceNumber: `${sequence.prefix}-${new Date(issue).getFullYear()}-${String(sequence.number).padStart(4, "0")}`,
      issueDate: issue,
    ...
    const created = createInvoiceWithItems(invoiceData, lines.map((line) => ({ invoiceId: 0, ...line })));
    const inv = created.invoice;
    await audit(req, "opret", "invoice", inv.id, inv.invoiceNumber);
    res.status(201).json({ ...inv, items: created.items });
  }));
  ...
  app.get("/api/invoices/:id/pdf", h(async (req, res) => {
    ...
    if (!inv) return res.status(404).json({ error: "Fakturaen blev ikke fundet." });
    if (req.auth?.role === "kunde" && inv.customerId !== req.auth.user.customerId) return res.status(404).json({ error: "Fakturaen blev ikke fundet." });
  ...
  // Backwards-compatible alias used by the original platform dashboard.
  app.get("/api/companies", requirePlatformAdmin, h(async (_req, res) => {
  ...
  app.post("/api/platform/companies", requirePlatformAdmin, h(async (req, res) => {
    ...
    const admin = await storage.createUser(validate(insertUserSchema, {
      ...
      password: hashPassword(String(adminPassword)), role: "leder", active: 1,
    }));
  ...
  app.get("/api/quotes", requireFeature("tilbud"), h(async (req, res) => {
    ...
    if (req.auth?.role === "kunde") return res.json(list.filter((row) => row.customerId === req.auth!.user.customerId));
    res.json(list);
  }));
  ...
  app.get("/api/quotes/:id", requireFeature("tilbud"), h(async (req, res) => {
    ...
    if (req.auth?.role === "kunde" && quote.customerId !== req.auth.user.customerId) return res.status(404).json({ error: "Tilbuddet blev ikke fundet." });
  ...
  app.post("/api/quotes", requireFeature("tilbud"), requireRole("leder", "holdleder", "platform_admin"), h(async (req, res) => {
    ...
    const sequence = allocateDocumentNumber(cid, "offer");
    const number = `${sequence.prefix}-${new Date().getFullYear()}-${String(sequence.number).padStart(4, "0")}`;
  ...
  app.get("/api/attachments/:id/fil", h(async (req, res) => {
    ...
    if (req.auth?.role === "assistent" || req.auth?.role === "kunde") {
      ...
      const allowed = req.auth.role === "assistent"
        ? task?.employeeId === req.auth.employeeId
        : task?.customerId === req.auth.user.customerId;
      if (!allowed) return res.status(404).json({ error: "Bilaget blev ikke fundet." });
    }
    if (att.storageKey) {
      const buf = await readFile(att.storage ?? "disk", att.storageKey);
      res.setHeader("Content-Type", att.mimeType ?? "application/octet-stream");
      const safeName = String(att.fileName ?? "bilag").replace(/[\r\n"\\]/g, "_");
  ...
  app.post("/api/platform/backups", requirePlatformAdmin, h(async (req, res) => {
    ...
    const artifact = await createEncryptedBackup(null);
    const item = await storage.insert("backupJobs", {
      companyId: null,
      scope: "platform",
      status: "fuldfort",
      size: `${(artifact.sizeBytes / 1024 / 1024).toFixed(2)} MB`,
      destination: "encrypted_local",
      autoSync,
  ...
  app.post("/api/backups", requireRole("leder", "platform_admin"), h(async (req, res) => {
    const cid = tenantId(req);
    const artifact = await createTenantBackup(cid);
    const item = await storage.insert("backupJobs", {
      companyId: cid,
      scope: "company",
      status: "fuldfort",
  ...
  app.post("/api/journal-entries", requireRole("leder", "platform_admin"), h(async (req, res) => {
    ...
    if (rawLines.length < 2) return res.status(400).json({ error: "En postering kræver mindst to linjer." });
    const lines = [];
    for (const raw of rawLines) {
      const parsedLine = validate(insertJournalLineSchema, { ...raw, companyId: cid, journalEntryId: 1 });
      const line = { ...parsedLine, debit: Number(parsedLine.debit ?? 0), credit: Number(parsedLine.credit ?? 0) };
      if (line.debit < 0 || line.credit < 0 || (line.debit > 0) === (line.credit > 0)) {
        return res.status(400).json({ error: "Hver linje skal have enten et positivt debet- eller kreditbeløb." });
      }
      if (!await storage.get("accounts", line.accountId, cid)) return res.status(400).json({ error: "En konto på posteringen findes ikke i virksomheden." });
      lines.push(line);
    }
    const debit = round2(lines.reduce((sum, line) => sum + line.debit, 0));
    const credit = round2(lines.reduce((sum, line) => sum + line.credit, 0));
    if (debit !== credit) return res.status(400).json({ error: "Posteringen balancerer ikke mellem debet og kredit." });
    const data = validate(insertJournalEntrySchema, { ...req.body, status: "kladde" });
    const item = createJournalWithLines(
      { ...data, companyId: cid, createdAt: new Date().toISOString() },
      lines.map((line) => ({ ...line, companyId: cid })),
    );
  ...
  app.patch("/api/journal-entries/:id", requireRole("leder", "platform_admin"), h(async (req, res) => {
    ...
    if (!existing) return res.status(404).json({ error: "Posteringen blev ikke fundet." });
    if (existing.status !== "kladde") return res.status(409).json({ error: "En bogført postering er uforanderlig." });
    const requestedStatus = String((req.body as any)?.status ?? "kladde");
    if (!['kladde', 'bogført'].includes(requestedStatus)) return res.status(400).json({ error: "Ugyldig status." });
    res.json(await storage.update("journal_entries", id, { status: requestedStatus }, cid));
  }));
  app.delete("/api/journal-entries/:id", requireRole("leder", "platform_admin"), h(async (req, res) => {
    ...
    const existing = await storage.get("journal_entries", id, cid);
    if (!existing) return res.status(404).json({ error: "Posteringen blev ikke fundet." });
    if (existing.status !== "kladde") return res.status(409).json({ error: "En bogført postering kan ikke slettes." });
    await storage.delete("journal_entries", id, cid);
  ...
  app.post("/api/backup-settings/:id/test", requireRole("leder", "platform_admin"), h(async (req, res) => {
    ...
    const existing = await storage.get("backup_settings", Number(req.params.id), cid);
    if (!existing) return res.status(404).json({ error: "Backup-indstilling blev ikke fundet." });
    if (existing.provider !== "local") return res.status(501).json({ error: "Denne cloududbyder er endnu ikke implementeret. Vælg lokal krypteret backup." });
    const artifact = await createEncryptedBackup(cid);
    const ok = artifact.sizeBytes > 0;
  ...
  app.post("/api/backup-settings/:id/sync", requireRole("leder", "platform_admin"), h(async (req, res) => {
    ...
    if (existing.provider !== "local") return res.status(501).json({ error: "Cloudsynkronisering er endnu ikke implementeret. Brug lokal krypteret backup." });
    const artifact = await createEncryptedBackup(cid);
    const size = `${(artifact.sizeBytes / 1024 / 1024).toFixed(2)} MB`;
    await storage.update("backup_settings", existing.id, {
    ...
    const job = await storage.insert("backup_jobs", {
      ...
      size, destination: "encrypted_local", autoSync: 0, summary: JSON.stringify(artifact), createdBy: req.auth?.user?.email || null, createdAt: nowIso(),
    });
  ...
  app.post("/api/lead-integrations/:id/test", requireRole("leder", "platform_admin"), h(async (req, res) => {
    ...
    const existing = await storage.get("lead_integrations", Number(req.params.id), tid);
    if (!existing) return res.status(404).json({ error: "Integration blev ikke fundet." });
    res.status(501).json({ error: `Forbindelsen til ${existing.provider} er ikke implementeret endnu.` });
  }));
  app.post("/api/lead-integrations/:id/import", requireRole("leder", "platform_admin"), h(async (req, res) => {
    const tid = tenantId(req);
    const existing = await storage.get("lead_integrations", Number(req.params.id), tid);
    if (!existing) return res.status(404).json({ error: "Integration blev ikke fundet." });
    res.status(501).json({ error: `Import fra ${existing.provider} er ikke implementeret endnu.` });
  }));
  ...
  app.get("/api/task-sessions", h(async (req, res) => {
    ...
    if (!["leder", "holdleder", "platform_admin"].includes(req.auth!.role)) {
      sessions = sessions.filter((s: any) => s.userId === req.auth!.userId);
    }
  ...
  app.post("/api/task-sessions/:id/pause", h(async (req, res) => {
    ...
    if (!["leder", "holdleder", "platform_admin"].includes(req.auth!.role) && s.userId !== req.auth!.userId) return res.status(403).json({ error: "Ingen adgang." });
  ...
  app.post("/api/export", h(async (req, res) => {
  ...
  app.get("/api/export/full", h(async (req, res) => {
    const cid = tenantId(req);
  ...
  // ═══════════════════════════════════════════════════════════════
  app.get("/api/cloud-providers", requirePlatformAdmin, h(async (_req, res) => {
    const providers = await storage.all("cloud_providers");
    res.json(providers.map(({ credentials: _secret, ...provider }: any) => ({ ...provider, hasCredentials: Boolean(_secret) })));
  }));

  app.post("/api/cloud-providers", requirePlatformAdmin, h(async (req, res) => {
    if (req.body?.provider !== "local") return res.status(501).json({ error: "Cloududbydere er endnu ikke implementeret. Vælg lokal krypteret backup." });
    const data = validate(insertCloudProviderSchema, { ...req.body, credentials: null, createdAt: nowIso() });
    const provider = await storage.insert("cloud_providers", data);
    const { credentials: _secret, ...publicProvider } = provider;
    res.status(201).json({ ...publicProvider, hasCredentials: Boolean(_secret) });
  }));

  app.patch("/api/cloud-providers/:id", requirePlatformAdmin, h(async (req, res) => {
    const id = Number(req.params.id);
    const provider = await storage.update("cloud_providers", id, req.body);
    if (!provider) return res.status(404).json({ error: "Udbyder ikke fundet." });
    const { credentials: _secret, ...publicProvider } = provider;
    res.json({ ...publicProvider, hasCredentials: Boolean(_secret) });
  }));
  ...
  app.post("/api/cloud-providers/:id/connect", requirePlatformAdmin, h(async (req, res) => {
    ...
    if (!provider) return res.status(404).json({ error: "Udbyder ikke fundet." });
    if (provider.provider !== "local") return res.status(501).json({ error: "Cloudforbindelsen er ikke implementeret endnu. Brug lokal krypteret backup." });
  ...
  app.post("/api/document-inbox", h(async (req, res) => {
    ...
    // Regelbaseret kategoriforslag fra filnavnet. Det markeres ikke som OCR,
    // fordi ingen ekstern OCR-motor er konfigureret her.
    if (data.fileName) {
      ...
      data.ocrData = JSON.stringify({ engine: "filename_rules", requiresReview: true, suggestedAt: nowIso() });
    }
    // Dubletkontrol
