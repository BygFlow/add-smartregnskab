...
export function assertDatabaseReady(): void {
  ...
  if (result?.ok !== 1) throw new Error("Databasen svarede ikke korrekt.");
}

// A clean deployment must be able to create its own schema.  Legacy databases
// predate the migration journal, so they are left untouched; all new databases
// are initialized from the checked-in, versioned migrations.
const hasApplicationSchema = Boolean(
  sqlite.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'companies'").get(),
);
if (!hasApplicationSchema) {
  ...
  sqlite.transaction(() => {
    ...
    addColumn("plans", "max_documents", "integer DEFAULT 500 NOT NULL");
...
export class DatabaseStorage implements IStorage {
  ...
  private getTable(name: string): any {
    if (!this.tableMap[name]) {
      const mod = require("@shared/schema");
      this.tableMap[name] = mod[name] || mod[name.replace(/_([a-z])/g, (_: string, c: string) => c.toUpperCase())];
    }
  ...
  async update(name: string, id: number, data: any, companyId?: number | null): Promise<any> {
    const table = this.getTable(name);
    if (!table) throw new Error(`Unknown table: ${name}`);
    let q: any = db.update(table).set(data);
    if (companyId !== undefined) {
      if (companyId === null) {
