...
const JOBS: Record<string, { label: string; everyMinutes: number; run: () => Promise<JobResult> }> = {
  gentagne_opgaver: { label: "Læg gentagne opgaver frem", everyMinutes: 60 * 6, run: jobRecurringTasks },
  faktura_rykkere: { label: "Rykkere på kundefakturaer", everyMinutes: 60 * 12, run: jobInvoiceReminders },
  abonnement_fornyelse: { label: "Fornyelse og opkrævning af abonnementer", everyMinutes: 60 * 12, run: jobSubscriptions },
  proeveperiode_paamindelse: { label: "Påmindelse om prøveperiode", everyMinutes: 60 * 24, run: jobTrialReminders },
  gdpr_oprydning: { label: "GDPR-oprydning efter opbevaringspolitik", everyMinutes: 60 * 24, run: jobRetention },
  oprydning: { label: "Fjern udløbne sessioner og tokens", everyMinutes: 60 * 6, run: jobCleanup },
  beskedkoe: { label: "Nyt forsøg på fejlede beskeder", everyMinutes: 30, run: jobOutbox },
