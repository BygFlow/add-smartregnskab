...
export async function generateSaft(companyId: number, startDate: string, endDate: string, userId: string) {
  ...
  return { xml: xml.replace("<SoftwareVersion>3.2.1</SoftwareVersion>", "<SoftwareVersion>3.5.1</SoftwareVersion>"), filename: `SAF-T-${cvr}-${startDate}-${endDate}.xml`, summary: { version: SAFT_VERSION, accounts: accountRows.length, entries: selectedEntries.length, lines: selectedLines.length, totalDebit, totalCredit } };
}
