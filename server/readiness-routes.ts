...
export function registerReadinessRoutes(app: Express) {
  ...
  app.get("/api/operations/status", requireRole("platform_admin"), asyncRoute(async (_req, res) => {
    ...
    res.json({ generatedAt: new Date().toISOString(), version: "3.15.2", overall: blocking.length ? "blocked" : services.some((service) => service.status === "warning") ? "attention" : "operational", services, blockers: blocking.map((service) => service.message) });
  }));
}
