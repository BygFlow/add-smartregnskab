...
import {
  ...
  Mail, Menu, Phone, PlayCircle, Receipt, ShieldCheck, Sparkles, Users, X,
} from "lucide-react";
...
type MarketingPage = "home" | "features" | "pricing" | "integrations" | "security" | "faq" | "about" | "contact" | "demo" | "help" | "guides" | "guide" | "privacy" | "terms" | "subscriptionTerms" | "payment" | "cookies" | "dpa" | "subprocessors" | "retention" | "landing";
...
function Footer() {
  return <footer className="bg-emerald-950 text-emerald-50"><div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 md:grid-cols-4">
    ...
    <div><p className="text-sm font-semibold">Virksomhed</p><div className="mt-3 space-y-2 text-sm text-emerald-100/70"><Link className="block" href="/om">Om produktet</Link><Link className="block" href="/privatliv">Privatliv</Link><Link className="block" href="/databehandleraftale">Databehandleraftale</Link><Link className="block" href="/underdatabehandlere">Underdatabehandlere</Link><Link className="block" href="/dataopbevaring-og-sletning">Dataopbevaring og sletning</Link><Link className="block" href="/vilkaar">Vilkår</Link><Link className="block" href="/abonnementsbetingelser">Abonnementsbetingelser</Link><Link className="block" href="/betaling-og-refusion">Betaling og refusion</Link><Link className="block" href="/cookies">Cookies</Link></div></div>
  </div><div className="border-t border-white/10 px-4 py-5 text-center text-xs text-emerald-100/55">© {new Date().getFullYear()} ADD SmartDrift ApS · CVR 46761898</div></footer>;
}
...
function SubscriptionTerms() {
  ...
  const sections: [string, string][] = [
    ...
    ["Betaling og automatisk fornyelse", "Abonnementet betales månedligt forud og fornyes automatisk én måned ad gangen. Kunden giver ved oprettelsen QuickPay og den tilknyttede kortindløser tilladelse til at gennemføre tilbagevendende betalinger, indtil abonnementet opsiges. ADD SmartRegnskab opbevarer ikke kortnummer eller kontrolcifre."],
    ["Opsigelse", "Abonnementet kan opsiges i løsningen. Opsigelsen har virkning ved udløbet af den allerede betalte abonnementsperiode, hvorefter der ikke gennemføres flere automatiske opkrævninger. Allerede betalte perioder refunderes som udgangspunkt ikke."],
    ["Prøveperiode", "Hvis en prøveperiode tilbydes, vises længden inden oprettelse. Betaling starter kun, når kunden udtrykkeligt opretter betalingsaftalen og accepterer disse abonnementsbetingelser."],
    ["Manglende betaling", "Ved afvist eller manglende betaling kan adgangen begrænses efter påmindelse. Kunden kan fortsat få adgang til lovpligtige dataudtræk efter de gældende produkt- og opbevaringsvilkår."],
    ["Pris- og vilkårsændringer", "Væsentlige ændringer varsles på et varigt medium inden ikrafttrædelsen. Kunden kan opsige abonnementet før ændringen træder i kraft. Ændringer, som alene er til kundens fordel eller følger ufravigelige krav, kan gennemføres uden samme varsel."],
    ["Kundens ansvar", "Kunden skal give korrekte virksomheds- og betalingsoplysninger, beskytte brugeradgange og kontrollere bogføring, indberetninger og betalinger. AI-forslag erstatter ikke kundens, bogholderens eller revisorens faglige kontrol."],
...
export function Marketing({ page, landingSlug, guideSlug }: { page: MarketingPage; landingSlug?: string; guideSlug?: string }) {
  ...
  if(page==="home")return <Home/>; if(page==="features")return <Features/>; if(page==="pricing")return <Pricing/>; if(page==="integrations")return <Integrations/>; if(page==="security")return <Security/>; if(page==="faq")return <Faq/>; if(page==="about")return <About/>; if(page==="contact")return <Contact/>; if(page==="demo")return <Contact demo/>; if(page==="help")return <Help/>; if(page==="guides")return <Guides/>; if(page==="guide")return <Guide slug={guideSlug??""}/>; if(page==="privacy")return <Legal kind="privacy"/>; if(page==="terms")return <Legal kind="terms"/>; if(page==="subscriptionTerms")return <SubscriptionTerms/>; if(page==="payment")return <Legal kind="payment"/>; if(page==="cookies")return <Legal kind="cookies"/>; if(page==="dpa")return <DpaPage/>; if(page==="subprocessors")return <SubprocessorsPage/>; if(page==="retention")return <RetentionPage/>; return <Landing slug={landingSlug??""}/>;
}
