...
const OFFICIAL_SOURCES = [
  {
    sourceKey: "dk-bogfoeringslov-vejledning",
    name: "Erhvervsstyrelsen — Bogføringsloven",
    url: "https://erhvervsstyrelsen.dk/vejledning-bogfoeringsloven",
    jurisdiction: "DK",
  },
  {
    sourceKey: "dk-registrerede-bogfoeringssystemer",
    name: "Erhvervsstyrelsen — Registrerede bogføringssystemer",
    url: "https://erhvervsstyrelsen.dk/fortegnelse-over-registrerede-bogfoeringssystemer",
    jurisdiction: "DK",
  },
  {
    sourceKey: "dk-skat-moms",
    name: "Skattestyrelsen — Moms",
    url: "https://skat.dk/erhverv/moms",
    jurisdiction: "DK",
  },
  {
    sourceKey: "dk-datatilsynet-ai",
    name: "Datatilsynet — Kunstig intelligens",
    url: "https://www.datatilsynet.dk/regler-og-vejledning/kunstig-intelligens",
    jurisdiction: "DK/EU",
  },
  {
    sourceKey: "eu-ai-act",
    name: "EUR-Lex — AI-forordningen",
    url: "https://eur-lex.europa.eu/eli/reg/2024/1689/oj?locale=da",
    jurisdiction: "EU",
  },
] as const;
...
export function ensureRegulatorySources(): void {
  ...
  for (const source of OFFICIAL_SOURCES) {
    const exists = db.select().from(regulatorySources)
      .where(eq(regulatorySources.sourceKey, source.sourceKey)).get();
  ...
  for (const source of sources) {
    try {
      ...
      if (parsed.protocol !== "https:" || !ALLOWED_HOSTS.has(parsed.hostname)) {
        throw new Error("Kilden er ikke på listen over tilladte officielle værter.");
      }
      const response = await fetch(source.url, {
        headers: {
          "user-agent": "ADD-SmartRegnskab-Regelovervaagning/1.0",
      ...
      const hash = normalizedHash(body);
      const now = new Date().toISOString();
