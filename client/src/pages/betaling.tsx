...
export default function Betaling() {
  ...
  const dunningNext = dunning === 1 ? "Hvis fakturaen fortsat ikke betales, sendes anden rykker, og virksomheden registreres i restance." : dunning === 2 ? "Hvis fakturaen fortsat ikke betales, spærres adgangen til ADD SmartRegnskab." : "Betal den forfaldne faktura for at få adgang igen.";
  ...
  return (
    <div className="p-4 md:p-4 space-y-3 max-w-6xl mx-auto">
      ...
      <SectionCard data-testid="card-payment-methods" icon={<CreditCard className="w-4 h-4" />} action={<Button size="sm" data-testid="button-add-payment-method" onClick={() => setAdding(true)}><Plus className="w-4 h-4 mr-1" />Tilføj</Button>} className="space-y-3">
        <div><h2 className="font-medium text-xs text-foreground">Betalingsmidler</h2><p className="text-[11px] text-muted-foreground mt-1">I demoen dannes et simuleret betalingsmiddel. Ved rigtig drift åbnes udbyderens sikre betalingsvindue — kortoplysninger gemmes aldrig i ADD SmartRegnskab.</p></div>
